    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white rounded-t-[28px] sm:rounded-[28px] p-5 max-h-[85vh] overflow-y-auto animate-slide-up">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-display font-extrabold text-brand-forest lowercase">create squad group</h3>
        ...
        <input
          ...
          className="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm font-bold text-brand-forest focus:outline-none focus:border-brand-grassFresh mb-4"
        />
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-xl border transition text-left ${
                  on ? 'border-brand-grassFresh tm-tint-green' : 'border-slate-100 hover:bg-slate-50'
                }`}
              >
                <img src={f.avatar} alt="" className="w-9 h-9 rounded-xl object-cover" />
                <span className="flex-1 font-bold text-sm text-brand-forest">{f.name}</span>
                {on && <Check className="w-4 h-4 text-brand-grassDeep" />}
              </button>
            );
          })}
        </div>
