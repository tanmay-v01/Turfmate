  ...
  return (
    <div className="p-4 lg:p-8 animate-fade-in max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-display font-extrabold text-white">Community Broadcasts</h2>
          <p className="text-sm font-bold text-slate-500">List tournament announcements, flash sales, and venue news</p>
        </div>
        <div className="flex gap-2">
          <div className="bg-white/5 border border-slate-200 px-3 py-1.5 rounded-xl text-center shadow-sm">
            <span className="text-[10px] font-bold text-slate-400 block uppercase">Views this Month</span>
            <span className="text-sm font-black text-white">4,200</span>
          </div>
          <div className="bg-white/5 border border-slate-200 px-3 py-1.5 rounded-xl text-center shadow-sm">
            <span className="text-[10px] font-bold text-slate-400 block uppercase">Conversions</span>
            <span className="text-sm font-black text-white">18 bookings</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {/* Form panel */}
        <form onSubmit={handlePublish} className="lg:col-span-7 bg-white/5 border border-slate-200 p-6 rounded-3xl shadow-sm space-y-4">
          <div className="flex items-center gap-3 pb-3 border-b border-white/10">
            ...
            <div>
              <h3 className="font-extrabold text-white text-sm">Campaign Builder</h3>
        ...
        <div className="lg:col-span-5 flex flex-col items-center">
          ...
          <div className="relative border-[10px] border-slate-900 rounded-[38px] shadow-2xl overflow-hidden w-[290px] h-[520px] bg-slate-100 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar pt-6 pb-4 px-2 space-y-3">
              ...
              <div className={`rounded-2xl bg-white/5 overflow-hidden shadow-sm border text-left ${
                category === 'PROMO'
                ...
                <div className="relative h-20 bg-slate-200">
                  ...
                  <span className="absolute bottom-1 left-2 px-1.5 py-0.5 rounded-full bg-white/90 text-[8px] font-bold text-white">
                ...
                <div className="p-3.5 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className="font-extrabold text-[10px] text-white truncate max-w-[120px]">{targetTurfObj?.name || 'Venue'}</span>
                  ...
                  {/* Headline & message */}
                  <div className="space-y-1 bg-slate-50/50 p-2 rounded-xl border border-white/10">
              ...
              <div className="rounded-2xl bg-white/5 border border-slate-200 p-3 space-y-2 opacity-50 pointer-events-none">
                <div className="flex items-center gap-2">
      ...
      <div className="border-t border-slate-200 pt-6">
        <h3 className="font-extrabold text-white text-sm mb-4">Active Campaigns</h3>
        {loadingCampaigns ? (
          <div className="bg-white/5 border border-slate-200 p-8 rounded-2xl text-center text-slate-400 text-xs font-bold">
            Loading campaigns…
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {ownerCampaigns.map(ann => {
              ...
              return (
                <div key={ann.id} className={`bg-white/5 border p-4 rounded-2xl flex items-start gap-3 relative ${
                  isPromoAnn ? 'border-amber-200 bg-amber-50/5' : 'border-slate-200'
                }`}>
                  <span className="text-2xl mt-1">{ann.sportIcon || '📢'}</span>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-bold text-white text-sm truncate">{ann.turfName || 'Venue Announcement'}</p>
