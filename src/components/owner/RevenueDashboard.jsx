  ...
  return (
    <div className="p-4 lg:p-8 animate-fade-in max-w-7xl mx-auto space-y-8">
      <div className="dash-hero !p-5">
        <h2 className="text-2xl font-display font-extrabold text-brand-forest">Revenue & Payouts</h2>
        <p className="text-sm font-bold text-brand-muted mt-1">Live from TurfMate bookings · 10% fee · You keep 90%</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="dash-stat-card bg-white border-brand-border/60">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Gross Collected</p>
          <p className="text-3xl font-black text-slate-800">₹{gross.toLocaleString()}</p>
          <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase">From app bookings</p>
        </div>
        <div className="dash-stat-card bg-white border-red-100">
        ...
        <div className="dash-stat-card bg-gradient-to-br from-brand-grassPale to-white border-brand-grassFresh/40 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10"><IndianRupee className="w-16 h-16 text-brand-forest" /></div>
          <p className="text-xs font-bold text-brand-forest uppercase tracking-wider mb-2">Your Net (90%)</p>
          <p className="text-3xl font-black text-brand-forest">₹{net.toLocaleString()}</p>
          <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase">Owner payout</p>
        </div>
        <div className="dash-stat-card bg-white border-amber-200/60 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-extrabold text-brand-forest">Transaction Ledger</h3>
              <button className="text-xs font-bold text-slate-500 flex items-center gap-1 hover:text-brand-forest transition">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {transactions.map(t => (
                      <tr key={t.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="p-4">
                          <div className="font-bold text-brand-forest text-sm">{t.id}</div>
                        ...
                        <td className="p-4 text-right font-black text-sm text-brand-forest">₹{t.net}</td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h3 className="font-extrabold text-brand-forest mb-6 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-brand-primary" /> Business Analytics</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase mb-2">Booking Volume</p>
                <div className="flex items-end gap-2 mb-2">
                  <span className="text-3xl font-black text-brand-forest">{bookings.length}</span>
                  <span className="text-xs font-bold text-green-500 mb-1">app bookings</span>
                </div>
                <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-primary rounded-full" style={{ width: `${Math.min(100, bookings.length * 12)}%` }}></div>
                </div>
              </div>
              <div>
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-extrabold text-brand-forest">Settlements</h3>
              ...
              {settlements.length === 0 ? (
                ...
                <div key={s.id} className="relative pl-6">
                  ...
                  <div className="flex justify-between items-start mb-1">
                    <p className="font-bold text-sm text-brand-forest">₹{s.amount.toLocaleString()} <span className="text-xs text-slate-400 font-normal">({s.date})</span></p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50">
              <h3 className="font-extrabold text-brand-forest">Tax Invoices (GST)</h3>
              ...
              {['June 2026', 'May 2026'].map((month, i) => (
                <div key={i} className="p-4 flex items-center justify-between hover:bg-slate-50 transition cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-sm text-brand-forest">{month}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Commission Tax Invoice</p>
                    </div>
                  </div>
                  <Download className="w-4 h-4 text-slate-400 hover:text-brand-forest transition" />
                </div>
              ))}
            </div>
          </div>
