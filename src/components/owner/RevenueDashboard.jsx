  ...
  return (
    <div className="p-4 lg:p-8 animate-fade-in max-w-7xl mx-auto space-y-8">
      <div className="dash-hero !p-5">
        <h2 className="text-2xl font-display font-extrabold text-slate-800">Revenue & Payouts</h2>
        <p className="text-sm font-bold text-slate-500 mt-1">Live from TurfMate bookings · 10% fee · You keep 90%</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="dash-stat-card bg-white border-slate-200/60">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Gross Collected</p>
          <p className="text-3xl font-black text-slate-700">₹{gross.toLocaleString()}</p>
        ...
        <div className="dash-stat-card bg-gradient-to-br from-brand-grassPale to-white border-brand-grassFresh/40 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10"><IndianRupee className="w-16 h-16 text-slate-800" /></div>
          <p className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">Your Net (90%)</p>
          <p className="text-3xl font-black text-slate-800">₹{net.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="font-extrabold text-slate-800">Transaction Ledger</h3>
              <button className="text-xs font-bold text-slate-500 flex items-center gap-1 hover:text-slate-800 transition">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {transactions.map(t => (
                      <tr key={t.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="p-4">
                          <div className="font-bold text-slate-800 text-sm">{t.id}</div>
                        ...
                        <td className="p-4 text-right font-black text-sm text-slate-800">₹{t.net}</td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h3 className="font-extrabold text-slate-800 mb-6 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-brand-primary" /> Business Analytics</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${Math.min(100, bookings.length * 12)}%` }}></div>
                </div>
              </div>
              <div>
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="font-extrabold text-slate-800">Settlements</h3>
              ...
              {settlements.length === 0 ? (
                ...
                <div key={s.id} className="relative pl-6">
                  ...
                  <div className="flex justify-between items-start mb-1">
                    <p className="font-bold text-sm text-slate-800">₹{s.amount.toLocaleString()} <span className="text-xs text-slate-400 font-normal">({s.date})</span></p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-200 bg-slate-50">
              <h3 className="font-extrabold text-slate-800">Tax Invoices (GST)</h3>
              ...
              {['June 2026', 'May 2026'].map((month, i) => (
                <div key={i} className="p-4 flex items-center justify-between hover:bg-slate-50 transition cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-sm text-slate-800">{month}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Commission Tax Invoice</p>
                    </div>
                  </div>
                  <Download className="w-4 h-4 text-slate-400 hover:text-slate-800 transition" />
                </div>
              ))}
            </div>
          </div>
