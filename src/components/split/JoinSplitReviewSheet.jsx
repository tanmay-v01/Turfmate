...
export default function JoinSplitReviewSheet() {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in">
      ...
      />
      <div className="relative w-full sm:max-w-md bg-white rounded-t-[28px] sm:rounded-[28px] shadow-premium border border-slate-100 animate-slide-up overflow-hidden">
        <div className="p-5 space-y-4">
          ...
          <div className="tm-card p-4 space-y-2">
            <p className="font-display font-extrabold text-brand-forest">{ann.turfName}</p>
          ...
          <div className="flex justify-between items-center bg-slate-50 rounded-2xl p-4 border border-slate-100">
            <div>
              ...
              <p className="text-2xl font-display font-extrabold text-brand-forest">₹{ann.costPerHead}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] text-slate-400 font-bold flex items-center gap-1 justify-end">
                <ShieldCheck className="w-3.5 h-3.5 text-brand-grassDeep" /> escrow
              </p>
          ...
          {app.isProcessingPayment ? (
            <div className="flex flex-col items-center py-6 gap-2">
              ...
              <p className="text-sm font-bold text-brand-forest">processing payment…</p>
            </div>
          ) : (
            <button
              type="button"
