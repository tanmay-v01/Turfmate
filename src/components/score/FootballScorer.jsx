    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-[24px] border border-slate-200 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <span className="text-[10px] font-black uppercase tracking-wider text-emerald-600">{periodLabel}</span>
          <span className="text-2xl font-mono font-bold text-slate-800">{formatTimer(game.timerSeconds)}</span>
        </div>
          ...
          {['first', 'half', 'second', 'full'].map((p) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase transition ${
                game.period === p ? 'bg-brand-forest text-slate-800' : 'bg-slate-100 text-slate-500'
              }`}
            >
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          className="py-5 rounded-2xl bg-emerald-50 border border-brand-grassFresh/40 font-display font-extrabold text-slate-800 hover:bg-brand-grassFresh/20 transition"
        >
      ...
      {game.events.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-4 max-h-36 overflow-y-auto">
...
function TeamScore({ team, onGoal, onUndo, align }) {
  return (
    <div className={`flex flex-col items-center gap-2 ${align === 'right' ? 'items-end' : ''}`}>
      ...
      <p className="text-5xl font-display font-extrabold text-slate-800">{team.goals}</p>
