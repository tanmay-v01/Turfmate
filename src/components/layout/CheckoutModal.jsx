...
export default function CheckoutModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in">
      ...
      <div className="relative w-full sm:max-w-md glass-grass rounded-t-[32px] sm:rounded-[32px] shadow-premium border border-slate-200/80 overflow-hidden animate-slide-up max-h-[92vh] overflow-y-auto">
        <div className="p-5 sm:p-6 space-y-5">
          <div className="flex justify-between items-start gap-3">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <TurfImage turf={app.activeTurf} className="w-14 h-14 rounded-xl object-cover shrink-0 border border-slate-200" />
              <div>
                <span className="tm-pill text-[10px] mb-1.5">secure checkout</span>
                <h3 className="font-display font-extrabold text-lg text-slate-800 lowercase">book {app.activeTurf.name}</h3>
            ...
            <button disabled={app.isProcessingPayment} onClick={() => app.setShowCheckoutModal(false)} className="p-2 rounded-xl hover:bg-emerald-50 text-slate-500 disabled:opacity-30">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              >
                <opt.icon className={`w-5 h-5 mx-auto mb-1.5 ${app.checkoutOption === opt.id ? 'text-emerald-600' : ''}`} />
                <span className="text-xs font-extrabold block text-slate-800">{opt.label}</span>
          ...
          <div className="tm-card p-4 space-y-2.5 text-sm">
            <div className="flex justify-between font-bold text-slate-500"><span>slot price</span><span className="text-slate-800">₹{slotPrice}</span></div>
            ...
            {isSplit && (
              <div className="pt-3 space-y-3 border-t border-slate-200/40">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-800">total players (incl. host)</span>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-slate-800 w-6 text-center">{Math.max(2, app.splitPlayersCount)}</span>
                ...
                <div className="flex gap-2">
                  {['public', 'private'].map(v => (
                    <button key={v} onClick={() => setSplitVisibility(v)} className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase border transition ${splitVisibility === v ? 'border-brand-grassFresh tm-tint-green text-slate-800' : 'border-slate-200 text-slate-500'}`}>
            ...
            <div className="flex justify-between items-center pt-3 border-t-2 border-brand-grassFresh/30">
              <span className="font-extrabold text-slate-800">due now</span>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <button
                ...
                className={`py-2 rounded-xl text-xs font-bold border transition ${paymentMethod === m.id ? 'border-brand-grassFresh tm-tint-green text-slate-800' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}
              >
                {m.label}
              </button>
            ))}
          ...
          {app.isProcessingPayment ? (
            <div className="flex flex-col items-center py-10 gap-4 animate-fade-in">
              ...
              <p className="text-xs font-bold text-slate-800 animate-pulse uppercase tracking-wider">{loadingMessages[loadingStep]}</p>
