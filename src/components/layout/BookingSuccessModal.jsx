  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden animate-pop">
        <div className="bg-gradient-to-br from-brand-grassPale to-white px-5 pt-5 pb-4 border-b border-brand-border">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="w-10 h-10 rounded-2xl bg-brand-grassFresh/20 flex items-center justify-center">
                <CheckCircle2 className="w-6 h-6 text-brand-grassDeep" />
              </span>
              <div>
                <p className="text-[10px] font-black uppercase tracking-wider text-brand-grassDeep">Payment successful</p>
                <h2 className="text-lg font-display font-extrabold text-brand-forest">Your slot is booked!</h2>
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="p-5 space-y-4">
          <div className="rounded-2xl border border-green-100 bg-green-50/80 p-4">
            ...
            <h3 className="font-bold text-brand-forest mt-2">{data.turfName}</h3>
          ...
          <div className="flex flex-col items-center p-4 bg-white border border-slate-100 rounded-2xl">
          ...
          <button
            type="button"
            onClick={() => app.setBookingSuccessData(null)}
            className="w-full py-2.5 text-sm font-bold text-slate-500 hover:text-brand-forest transition"
          >
            Done
          </button>
        </div>
