      ...
      {app.showNotifications && (
        <div className="fixed inset-0 z-50 lg:pl-[252px]">
          ...
          <motion.div 
            ...
            className="absolute top-0 right-0 w-full max-w-md h-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-premium flex flex-col border-l border-slate-200 dark:border-slate-800"
          >
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800">
              <h3 className="font-display font-extrabold text-lg text-slate-800 dark:text-brand-grassLight flex items-center gap-2 lowercase">
                <Bell className="w-5 h-5 text-emerald-600" /> pings
              </h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {app.notifications.length === 0 ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 px-4">
                  ...
                  <p className="font-bold text-slate-800 dark:text-slate-700">All caught up</p>
                ...
                <motion.button
                  ...
                  className={`w-full text-left p-4 rounded-[20px] border transition-all duration-300 ${
                    n.read 
                      ...
                      : 'bg-emerald-50 dark:bg-brand-forest/20 border-brand-grassFresh/40 dark:border-brand-grassDeep/40 shadow-soft'
                  }`}
                >
                  <p className={`text-sm font-medium ${n.read ? 'text-slate-500 dark:text-slate-400' : 'text-slate-800 dark:text-brand-grassLight'}`}>{n.text}</p>
