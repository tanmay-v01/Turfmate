      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {!app.onboardingData.kycDoc && !app.uploadingFile ? (
            <div className="tm-card border-dashed border-2 border-slate-200 p-6 text-center">
              <Plus className="w-8 h-8 text-slate-500 mx-auto mb-2" />
              <p className="text-sm font-bold text-slate-800">upload document</p>
              <div className="flex gap-2 justify-center mt-4">
                <button type="button" onClick={() => simulateUpload('shop_act_license.pdf (1.2MB)', 20)} className="px-4 py-2 bg-emerald-50 rounded-full text-xs font-bold text-emerald-600 hover:bg-brand-grassLight transition">PDF</button>
                <button type="button" onClick={() => simulateUpload('pan_card_scan.jpg (800KB)', 25)} className="px-4 py-2 bg-emerald-50 rounded-full text-xs font-bold text-emerald-600 hover:bg-brand-grassLight transition">JPG</button>
              </div>
            </div>
          ) : app.uploadingFile ? (
            ...
            <div className="tm-card p-4 flex justify-between items-center bg-emerald-50/40">
              <div className="flex items-center gap-3">
                <span className="text-2xl">📄</span>
                <div>
                  <p className="font-bold text-sm text-slate-800 truncate max-w-[200px]">{app.onboardingData.kycDoc}</p>
