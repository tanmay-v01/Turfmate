    ...
    <div className="min-h-screen dash-owner-bg relative flex flex-col">
      ...
      <div className="flex-grow flex flex-col justify-center p-6 max-w-md mx-auto w-full animate-fade-in relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-display font-extrabold text-brand-forest mt-4">Almost there!</h2>
          <p className="text-sm text-brand-muted font-bold mt-2 leading-relaxed">
            Hi <b className="text-brand-forest">{app.userProfile.ownerName || app.userProfile.name}</b> —{' '}
            <b className="text-brand-forest">{app.userProfile.businessName}</b> is under review.
            TurfMate takes 10% per booking; you keep 90%.
          </p>
        </div>

        <div className="dash-card p-5 space-y-3 mb-6">
          <p className="text-[10px] font-extrabold text-brand-muted uppercase tracking-wider flex items-center gap-2">
            <Shield className="w-3.5 h-3.5" /> Verification Progress
          </p>
          {checks.map(c => (
            <div key={c.label} className="flex items-center justify-between text-sm">
              <span className="font-bold text-brand-forest/80">{c.label}</span>
              {c.status === 'done' && <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-md bg-brand-grassPale text-brand-grassDeep">✓ Done</span>}
              {c.status === 'skip' && <span className="text-[10px] font-bold text-brand-muted italic">Optional</span>}
        ...
        <div className="space-y-3">
          ...
          <p className="text-[10px] text-center text-brand-muted font-bold">In production, TurfMate super admin approves from God Mode panel</p>
          <button
            onClick={() => { localStorage.removeItem('tm_profile'); app.setUserProfile({ isLoggedIn: false }); app.setView('welcome_carousel'); }}
            className="w-full py-2.5 text-brand-muted font-bold text-xs hover:text-brand-forest transition"
          >
            Cancel & restart
          </button>
        </div>
