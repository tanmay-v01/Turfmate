...
export default function SplashPage() {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-[#090D19]">
      <GrassBackground />
      <img
      ...
      {FLOATERS.map((emoji, i) => (
        <span
          key={emoji}
          className="absolute text-2xl sm:text-3xl opacity-20 animate-float pointer-events-none select-none"
          style={{
            top: `${15 + (i * 13) % 70}%`,
            left: `${8 + (i * 17) % 84}%`,
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 animate-pop">
        ...
        <h1 className="text-5xl sm:text-6xl font-display font-extrabold text-white">
          Turf<span className="text-lime-400">Mate</span>
        </h1>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2 max-w-xs">
          {['virar', 'mumbai', 'squad up'].map((t) => (
            <span key={t} className="px-3 py-1 rounded-full text-[11px] font-bold bg-lime-400/10 text-lime-400 border border-lime-400/20">#{t}</span>
          ))}
        </div>
        <div className="mt-10 flex items-center gap-2">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="w-2.5 h-2.5 rounded-full bg-lime-400 animate-pulse-soft"
