        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {TABS.map(tab => (
            ...
            >
              ...
              {tab.badge > 0 && (
                <span className="bg-amber-500 text-slate-900 text-[10px] font-black px-1.5 py-0.5 rounded-full">{tab.badge}</span>
              )}
            </button>
          ))}
        </nav>
            ...
            <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
              ...
              {pendingOwners.length > 0 && (
                <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <button onClick={() => setActiveTab('owners')} className="px-5 py-2.5 bg-amber-400 text-slate-900 text-xs font-black rounded-xl hover:bg-amber-300 transition">
                    Review Now →
                  </button>
                </div>
              )}
                  ...
                  {pendingOwners.map(owner => (
                    <div key={owner.id} className="rounded-2xl border border-slate-700/80 bg-slate-800/50 p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4 hover:border-brand-grassFresh/20 transition">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => app.approveOwnerApplication(owner.id)} className="px-5 py-2.5 bg-brand-grassFresh text-slate-900 text-xs font-black rounded-xl hover:brightness-110 transition flex items-center gap-1.5 shadow-pill">
                    ...
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl p-6 shadow-2xl relative animate-slide-up text-left">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {!isBanned ? (
                          ...
                          <button onClick={() => { app.unbanUser(selectedUser.username); setSelectedUser(null); }} className="w-full py-3 bg-emerald-500 text-slate-900 font-extrabold rounded-xl text-xs hover:bg-emerald-450 transition uppercase tracking-wider">
                            Lift Ban
                          </button>
                        )}
                      </div>
      ...
      <nav className="dash-mobile-nav dash-mobile-nav-super md:hidden">
        {TABS.filter(t => t.mobile).map(tab => (
          ...
          >
            ...
            {tab.label}
            {tab.badge > 0 && <span className="absolute -top-0.5 right-1 w-4 h-4 bg-amber-500 text-slate-900 text-[8px] font-black rounded-full flex items-center justify-center">{tab.badge}</span>}
          </button>
        ))}
      </nav>
    </div>
