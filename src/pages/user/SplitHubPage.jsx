  ...
  if (!activeSplit || activeSplit.status === 'failed' || activeSplit.status === 'canceled') {
    return (
      <div className="flex flex-col items-center justify-center p-8 min-h-[60vh] space-y-4 bg-slate-50">
        ...
        <button onClick={() => app.setView('locker_room')} className="text-brand-forest font-bold">
          browse locker room →
        </button>
      </div>
    );
    ...
    <div className="animate-fade-up flex flex-col min-h-screen bg-slate-50 pb-36">
      <div className="bg-white border-b border-slate-100 px-4 py-3 sticky top-0 z-20 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-extrabold text-brand-forest leading-tight lowercase">split hub</h1>
            <span className="text-[10px] text-slate-400 font-semibold uppercase">{activeSplit.turfName}</span>
          </div>
        </div>
        <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-full ${isFilled ? 'tm-tint-green text-brand-forest' : 'bg-amber-100 text-amber-800'}`}>
      ...
      <div className="p-4 space-y-4 flex-grow overflow-y-auto">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
          <div className="flex justify-between items-end">
            <div>
              <span className="text-xs font-bold text-slate-800">funding progress</span>
              <span className="text-[10px] text-slate-500 block">{playersJoined}/{totalNeeded} players joined</span>
            </div>
            {!isFilled && (
              <span className="text-sm font-bold text-brand-forest">₹{amountRemaining} remaining</span>
            )}
          </div>
            ...
            {(activeSplit.roster || []).map((player, idx) => (
              <div key={player} className="p-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-xs font-bold text-slate-800 block">{player}</span>
                    <span className="text-[9px] text-slate-400">{idx === 0 ? 'host' : 'joined'}</span>
                  </div>
                </div>
                <span className="flex items-center gap-1 text-[10px] font-bold text-brand-forest tm-tint-green px-2 py-1 rounded border">
      ...
      <div className="fixed bottom-20 lg:bottom-4 inset-x-4 bg-white border border-slate-100 rounded-2xl p-4 flex gap-3 z-30 shadow-lg">
        <button
          onClick={handleShare}
