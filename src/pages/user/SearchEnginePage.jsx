    ...
    <div className="flex flex-col min-h-[70vh] text-left w-full relative">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-grow relative">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 tm-icon-btn !w-8 !h-8 !rounded-xl text-emerald-600"
              aria-label="Filters"
            >
      ...
      <div className="flex-grow flex flex-col lg:flex-row relative overflow-hidden min-h-[420px]">
        ...
        <div className={`w-full lg:w-[45%] xl:w-[40%] flex flex-col h-full overflow-y-auto p-4 space-y-3 pb-24 lg:pb-4 no-scrollbar ${app.searchViewMode === 'map' ? 'hidden lg:flex' : 'flex'}`}>
          {filteredList.length === 0 ? (
            <EmptyState
              ...
              className="my-10 border border-slate-200"
            />
          ) : (
            <AnimatePresence>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3 text-left flex-grow">
                      ...
                      <h4 className="font-extrabold text-sm text-slate-800 dark:text-brand-grassLight mt-1.5 truncate">{turf.name}</h4>
                      ...
                      <div className="flex justify-between items-center mt-3 pt-2 border-t border-slate-50 dark:border-slate-700/50">
                        ...
                        <span className="text-xs font-extrabold text-slate-800 dark:text-brand-grassLight">₹{turf.pricePerHour}</span>
                      </div>
                    </div>
                  </motion.div>
                );
        ...
        <div className={`w-full lg:w-[55%] xl:w-[60%] flex-grow relative bg-slate-100 overflow-hidden flex-col min-h-[50vh] lg:min-h-0 ${app.searchViewMode === 'list' ? 'hidden lg:flex' : 'flex'}`}>
          ...
          {app.selectedTurfForPreview && (
            <div className="absolute bottom-4 left-4 right-4 lg:left-auto lg:w-80 bg-white/95 backdrop-blur-xs p-3 rounded-2xl border border-slate-200 shadow-xl flex items-center gap-3 z-[500] animate-fade-in text-brand-text">
              <TurfImage turf={app.selectedTurfForPreview} className="w-16 h-16 rounded-xl object-cover" />
              <div className="flex-grow text-left min-w-0">
                <h4 className="font-extrabold text-xs text-slate-800 truncate">{app.selectedTurfForPreview.name}</h4>
                ...
                <span className="text-[10px] font-bold text-emerald-600 leading-none mt-1 block">
      ...
      {app.showFilterSheet && (
        <div className="absolute inset-0 bg-black/60 z-40 animate-fade-in flex items-end justify-center">
          <div className="w-full bg-white rounded-t-[32px] p-5 space-y-4 animate-slide-in shadow-2xl border-t border-slate-200 max-h-[80%] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-200 pb-3">
              <h3 className="font-extrabold text-sm text-slate-800 font-display uppercase tracking-wider">Search Filters</h3>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase">
                ...
                <span className="text-slate-800 font-bold">{app.filterTimeRange[0]}:00 - {app.filterTimeRange[1]}:00</span>
              </div>
              <input
                type="range"
                min="6"
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase">
                ...
                <span className="text-slate-800 font-bold">{app.filterRadius} km</span>
              </div>
              <input
                type="range"
                min="2"
