      ...
      <div className="glass-card p-4 sm:p-5 mb-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-2xl font-display font-extrabold text-brand-forest tabular-nums">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={stat.label} className="rounded-xl px-3 py-2.5 text-center border border-slate-200/90 bg-white/90">
              ...
              <p className="text-lg font-display font-extrabold text-brand-forest tabular-nums leading-none">
                {stat.value}
              </p>
        ...
        <div className="flex items-center gap-3 pt-1 border-t border-white/60">
          <span className="flex items-center justify-center w-9 h-9 rounded-xl tm-icon-accent-sky shrink-0">
            <Navigation2 className="w-4 h-4 text-brand-grassDeep" strokeWidth={2.25} />
          </span>
          ...
          <button
            ...
            className="shrink-0 text-[10px] font-bold text-brand-grassDeep uppercase tracking-wide hover:underline"
          >
            edit
          </button>
        </div>
        ...
        {app.hoveredMapPin && (
          <div className="absolute bottom-3 left-3 right-3 z-[500] glass-panel rounded-2xl p-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="font-bold text-sm text-brand-forest truncate">{app.hoveredMapPin.name}</p>
      ...
      <section className="space-y-3">
        ...
        {filteredTurfs.length === 0 ? (
          <div className="glass-card p-6 text-center">
            ...
            <p className="text-sm font-semibold text-brand-forest">No turfs in this radius</p>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm text-brand-forest truncate">{t.name}</p>
                  <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                    <MapPin className="w-3 h-3 shrink-0" /> {dist} km · {t.city}
                  </p>
                </div>
                <span className="text-sm font-bold text-brand-forest tabular-nums">₹{t.pricePerHour}</span>
              </button>
            );
          })
        )}
