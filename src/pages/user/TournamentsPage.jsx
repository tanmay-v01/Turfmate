...
export default function TournamentsPage() {
  ...
  return (
    <div className="animate-fade-up pb-12">
      ...
      {/* Tabs */}
      <div className="flex bg-white p-1 rounded-2xl border border-slate-200 gap-1 mb-6 shadow-sm">
        <button
          onClick={() => { setActiveTab('explore'); setSelectedTournament(null); }}
          className={`flex-1 py-3 text-xs font-black rounded-xl transition ${
            activeTab === 'explore' ? 'tm-chip-green' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-3 text-xs font-black rounded-xl transition relative ${
            activeTab === 'my-registrations' ? 'tm-chip-green' : 'text-slate-500 hover:text-slate-800'
          }`}
        >
          My Registrations
          {myRegistrations.length > 0 && (
            <span className="absolute top-2 right-4 w-4 h-4 bg-emerald-500 text-slate-800 text-[9px] font-black rounded-full flex items-center justify-center">
                ...
                <div className="h-40 relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-between p-4">
                    <div className="flex justify-between items-center">
                      <span className={`text-[9px] font-black tracking-wider uppercase px-2.5 py-1 rounded-full text-slate-800 ${
                ...
                <div className="p-4 space-y-4">
                  <div className="grid grid-cols-3 gap-2 bg-slate-50 p-3 rounded-2xl text-center">
                    <div>
                      ...
                      <p className="text-sm font-black text-slate-800">₹{t.prizePool.toLocaleString()}</p>
                    </div>
                    <div>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-slate-200">
                    <button
                      ...
                      className="flex-1 py-3 border border-slate-200 hover:border-brand-forest text-slate-700 hover:text-slate-800 text-xs font-black rounded-xl transition flex items-center justify-center gap-1.5"
                    >
                      <Award className="w-4 h-4" /> View Details
                    </button>

                    {registered ? (
                      <span className="flex-1 bg-emerald-50 text-slate-800 text-xs font-black py-3 rounded-xl flex items-center justify-center gap-1">
                        <CheckCircle2 className="w-4 h-4 text-slate-800" /> Registered
                      </span>
                    ) : t.status === 'open' ? (
                      <button
      ...
      {selectedTournament && (
        <div className="space-y-6">
          <div className="bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-sm p-5 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                <button
                  onClick={() => setSelectedTournament(null)}
                  className="text-xs font-bold text-slate-400 hover:text-slate-800 flex items-center gap-1 mb-2"
                >
                  &larr; Back to championships
                </button>
                <h2 className="text-xl font-black text-slate-800">{selectedTournament.name}</h2>
                ...
                <div className="flex min-w-[500px] justify-between items-center gap-4 py-2">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    {selectedTournament.brackets.quarter.map((q) => (
                      <div key={q.id} className="bg-white border border-slate-200 rounded-xl p-2 text-[10px] shadow-sm space-y-1">
                        <div className="flex justify-between items-center">
                  ...
                  <div className="flex-1 space-y-6">
                    ...
                    {selectedTournament.brackets.semi.map((s) => (
                      <div key={s.id} className="bg-white border border-slate-200 rounded-xl p-2 text-[10px] shadow-sm space-y-1">
                        <div className="flex justify-between items-center">
            ...
            <div className="pt-4 border-t border-slate-200 flex gap-2">
              ...
              {isAlreadyRegistered(selectedTournament.id) ? (
                <span className="flex-1 bg-emerald-50 text-slate-800 text-xs font-black py-3 rounded-xl flex items-center justify-center gap-1">
                  <CheckCircle2 className="w-4 h-4 text-slate-800" /> You are registered for this league
                </span>
        ...
        <div className="p-4">
          {myRegistrations.length === 0 ? (
            ...
            myRegistrations.map((reg, idx) => (
              <div key={idx} className="bg-white rounded-3xl overflow-hidden border border-slate-200 p-4 shadow-sm flex gap-4">
                ...
                <div className="min-w-0 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <h4 className="font-black text-slate-800 text-sm truncate mt-1">{reg.tournamentName}</h4>
                    <p className="text-[10px] text-slate-400 font-bold">Team: <span className="text-slate-800 font-black">{reg.teamName}</span></p>
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 border-b border-slate-200 flex justify-between items-center shrink-0">
              <div>
                <h3 className="font-black text-slate-800 text-sm">Register Team Squad</h3>
            ...
            <div className="p-5 overflow-y-auto space-y-4 flex-1">
              ...
              {registrationStep === 2 && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs text-slate-800 font-bold">{selectedSquad.length} selected</span>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 p-4 rounded-2xl space-y-2 border border-slate-200">
                    ...
                    <div className="flex justify-between items-center text-xs pt-2 border-t border-slate-200/80">
                      ...
                      <span className="text-sm font-black text-slate-800">₹{selectedTournament.entryFee}</span>
                    </div>
                  </div>

                  <div className="space-y-2.5">
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-wider">Select Payment Method</p>
                    <div className="border border-brand-primary bg-emerald-500/5 rounded-xl p-3.5 flex justify-between items-center">
                      <div className="flex items-center gap-2.5">
                        <span className="p-1.5 bg-emerald-50 rounded-lg text-slate-800 font-black text-xs">UPI</span>
                        <div className="text-left">
                          <p className="text-xs font-bold text-slate-700">PhonePe / Google Pay / BHIM</p>
                          <p className="text-[10px] text-slate-400 font-medium">Pay securely from any UPI app</p>
                        </div>
                      </div>
                      <span className="w-4 h-4 rounded-full bg-brand-forest flex items-center justify-center text-slate-800">
              ...
              {registrationStep === 4 && (
                <div className="text-center py-6 space-y-4">
                  <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center mx-auto text-slate-800 border-4 border-white shadow-md">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-left space-y-1.5">
